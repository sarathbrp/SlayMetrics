#!/bin/bash
# ============================================================
# Round 3 — Reset DUT to Clean State
...
echo "=========================================="
echo "RESETTING DUT (ROUND 3) TO CLEAN STATE"
echo "=========================================="

# ─── STOP MEMORY PRESSURE ──────────────────────────────────
echo ">>> Stopping memory pressure"
kill $(cat /tmp/memory_pressure_pid 2>/dev/null) 2>/dev/null || true
rm -f /tmp/memory_pressure_pid
# Kill any remaining stress-ng
killall stress-ng 2>/dev/null || true
# Clean tmpfs
rm -f /dev/shm/pressure 2>/dev/null || true

# ─── REMOVE TC RULES ───────────────────────────────────────
echo ">>> Removing tc traffic shaping"
...
systemctl set-property nginx.service IOWeight= 2>/dev/null || true
systemctl set-property nginx.service CPUWeight= 2>/dev/null || true
